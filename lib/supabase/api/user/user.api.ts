...
export class UserAPI implements IUser {
  ...
  async create(data: CreateUserDTO): Promise<Result<User, RepositoryError>> {
    ...
    .select().single()
    if (error) return Result.Err(RepositoryError.fromPostgresError(error, 'User'));
    if(!user) return Result.Err(new RepositoryError(RepositoryErrorCode.NotFound, 'Failed to create user'));
